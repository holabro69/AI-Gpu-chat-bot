...
<head>
  ...
  <style>
    ...
    #toggleMode, #filterPrice, #filterPSU, #filterTDP {
      margin-top: 10px;
      padding: 5px 10px;
      background: #444;
      color: #f0f0f0;
...
<body>
  <h2>all the gpu info bot 💬</h2>
  <div id="chatbox"></div>
  <input type="text" id="userInput" placeholder="Type a command like 'spec rtx 3080', 'rx 6600 xt vs rtx 3060', or 'help'">
  <div>
    <button id="toggleMode">Toggle Dark/Light Mode</button>
    <button id="filterPrice">💰 Filter Price</button>
    <button id="filterPSU">🔌 PSU Suggest</button>
    <button id="filterTDP">🔥 TDP Filter</button>
  </div>
  <script>
    const gpuSpecs = {}; // You can paste your full Object.assign() data here  
...
Object.assign(gpuSpecs, {
  ...
  if (msg.startsWith("spec ")) {
    ...
    if (spec) {
      ...
      return `❌ GPU "${name}" not found. Try typing exact name like 'rtx 3060' or 'rx 6600 xt'.`;
    }
  }
  ...
  if (msg.includes(" vs ")) {
    ...
    if (!spec1 || !spec2) return `❌ One or both GPUs not found. Try exact names.`;
  ...
  if (msg.startsWith("psu ")) {
    ...
    if (list.length === 0) return "❌ No GPUs found for that PSU.";
    return `🔌 GPUs for PSU ≤ ${watt}W:\n` + list.map(([name, g]) => `• ${name.toUpperCase()} (${g.psu})`).join("\n");
  }
  ...
  if (msg.startsWith("price ")) {
    ...
    if (list.length === 0) return "❌ No GPUs found under that price.";
    return `💰 GPUs under $${price}:\n` + list.map(([name, g]) => `• ${name.toUpperCase()} ($${g.price2025})`).join("\n");
  }
  ...
  if (msg.startsWith("tdp ")) {
    ...
    if (list.length === 0) return "❌ No GPUs found under that TDP.";
    return `🔥 GPUs with TDP ≤ ${tdpLimit}W:\n` + list.map(([name, g]) => `• ${name.toUpperCase()} (${g.tdp})`).join("\n");
  }
...
📦 spec <gpu> — Show detailed GPU specs (e.g., spec rtx 3060)
🔍 <gpu1> vs <gpu2> — Compare two GPUs (e.g., rx 6600 xt vs rtx 3060)
💰 price <amount> — Filter GPUs under a price (e.g., price 300)
🔌 psu <wattage> — List GPUs that can run on your PSU (e.g., psu 500w)
🔥 tdp <amount> — Show GPUs under a TDP limit (e.g., tdp 200)
🎭 joke — Get a tech joke
📢 test — Test if the bot is working
📋 credit — Show the creator and tech info
✅ nvidia / amd / intel — Quick brand tips`;
  }
  ...
  return "🤖 Type 'spec <gpu>', 'compare <gpu1> vs <gpu2>', or 'psu 500w' to get started.";
    ...
    document.getElementById("toggleMode").addEventListener("click", () => {
      ...
      const isDark = body.style.backgroundColor === "rgb(30, 30, 30)";
      body.style.backgroundColor = isDark ? "#f0f0f0" : "#1e1e1e";
      body.style.color = isDark ? "#000" : "#f0f0f0";
      document.getElementById("chatbox").style.backgroundColor = isDark ? "#fff" : "#2c2c2c";
    });
    document.getElementById("filterPSU").addEventListener("click", () => {
  const watt = prompt("Enter your PSU wattage (e.g. 500):");
  if (watt) {
    const msg = `psu ${watt}`;
    addMessage("user", msg);
    const reply = getBotReply(msg);
...
document.getElementById("filterPrice").addEventListener("click", () => {
  const price = prompt("Enter max price in USD (e.g. 300):");
  if (price) {
...
document.getElementById("filterTDP").addEventListener("click", () => {
  const tdp = prompt("Enter max TDP in watts (e.g. 200):");
  if (tdp) {
    const msg = `tdp ${tdp}`;
    addMessage("user", msg);
    const reply = getBotReply(msg);
  ...
  <div id="footer">
    🤖 Powered by <strong>ChatGPT</strong> | 🧠 Made by <strong>Holabro</strong>
  </div>
</body>
</html>
